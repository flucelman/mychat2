...
func (c *greeterClient) AIResponse(ctx context.Context, in *AIStreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[AIStreamResponseReply], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Greeter_ServiceDesc.Streams[0], Greeter_AIResponse_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[AIStreamRequest, AIStreamResponseReply]{ClientStream: stream}
...
func _Greeter_AIResponse_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(AIStreamRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(GreeterServer).AIResponse(m, &grpc.GenericServerStream[AIStreamRequest, AIStreamResponseReply]{ServerStream: stream})
}
...
var Greeter_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "AIResponse.Greeter",
